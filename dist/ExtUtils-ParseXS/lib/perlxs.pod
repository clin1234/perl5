...
accurately specify where line breaks can occur.
 ...
 alias             = "ALIAS:"
                       // One or more lines; each with 0, 1, or more
                       // {alias name - package optional, op, index}
                       // triplets:
                       (
                         [bar]       "="  [5]
...
the file. But if you do have that basic familiarity, then this document
may be read through in order: it explains the concepts first, then
presents the components of XS syntax in roughly the order they might
appear in an XS file. Alternatively, it may be used as a reference manual
to look up particular constructs.
...
L<perlguts>: this contains various details of the internals of the Perl
interpreter. A better understanding of that may help when writing more
complex XSUBs, or when debugging.
...
L<perlcall>: this describes how to call Perl functions (and eval etc) from
C.
...
wrappers to call existing C library functions, they may take a fixed list
of arguments and return a single result (or zero items for a C<void>
function).

An XS file is a template file format which contains a mixture of C and
XSUB declarations. It is used to generate XSUBs where most boilerplate
code is handled automatically: e.g. converting argument values between C
and Perl.

Note that this document refers to both the thing in the XS file, and the C
function text generated from it, as an XSUB. It should be clear from the
context which is being referred to.

When XSUBs are being used as a thin wrapper between perl and the functions
in a particular library, the XSUB definitions in the XS file are often
...
XSUBs have three main roles. They can be used as thin wrappers for library
functions, e.g. L<Digest::SHA>. They can be used to write Perl
functions which are faster than pure perl or easier to do in C and which
may accept or return lists of arguments, e.g. L<List::Util>. Or they can
be used to extend the Perl interpreter itself, e.g. L<threads>.
...
The L<h2xs> utility bundled with Perl can be used to generate an initial
XS file from a C header file, which (with possibly only minor edits) can
be used to wrap an entire C API. h2xs can also be used to generate an
initial skeleton distribution even when not deriving from a header file
...
There is a standard system typemap file for common C and Perl types, but
you can add your own typemap file in addition. From perl 5.16.0 onwards
you can also include extra typemap declarations inline within the XS file.
You can either just add new C to XS typemap mappings to make use of
existing templates, or you can add new templates too. As an example of the
former, if you're using a C header file which has:
...
An XS file is parsed by the L<xsubpp> utility (which is a thin wrapper
over L<ExtUtils::ParseXS>), and generates a C<.c> file. xsubpp is typically
called at build time from the Makefile of a distribution (as generated by
L<ExtUtils::MakeMaker>); or L<ExtUtils::ParseXS> can be used directly,
e.g. by L<Module::Build>. The C file is then compiled into a C<.so> or
C<.dll>, again at build time.
...
Anything before the first C<MODULE> directive line is treated as pure C.
All such lines (including C code comments) are passed through unprocessed
into the destination C file, except for any sections of POD, which are
discarded. C comments (C</* ... */>)  and XS comments (as described below)
aren't recognised, and are just passed through unprocessed.

It is very unlikely that human-generated C code in this half will be
mistaken for POD, as most indenting styles result in whitespace in front
of any line starting with C<=>. However, machine generated XS files may
fall into this trap unless care is taken to ensure that a space breaks the
sequence C<"\n=">.

This half of the file is the place to put things such as C<#include>,
C<#define> and C<typedef>, and to declare any (typically static) C helper
functions.
...
Any line following an XS comment line which ends with a backslash is
...
before the start of an XSUB. While some items I<are> processed if they are
on the line immediately before start of an XSUB, the parser is
inconsistent in their handling.
...
An C<OP> is is a data structure within the perl interpreter. It forms a
node within a tree structure created when the perl source is compiled. It
usually represents a single operation within the perl source, such as an
add or function call. The structure has various flags and data, and a
pointer to a C function (called the PP function) which is used to
implement the actions of that OP. The main loop of the perl interpreter
consists of calling the PP function of the current OP (C<PL_op>) and then
updating the variable which points to the current OP, C<PL_op>: typically
to C<< PL_op->op_next >>. In particular, the C<ENTERSUB> OP performs (or
at least starts) a function call.
...
variables, are stored in structures called I<SV>s. These SVs can hold data
of many different types, including integers (IV), strings (PV), references
(RV), arrays (AV), elements of arrays, etc. These will be discussed in more
detail below.
...
C<ENTERSUB> OP will pop that typeglob, extract the C<&foo> CV from it, and
see whether it is a normal CV or an XSUB CV. (A CV, or I<code value>, is a
type of SV which contains information about a subroutine).

For a normal Perl subroutine call, the C<ENTERSUB> OP will then: pop the
...
For an XSUB sub, the C<ENTERSUB> OP will instead note the value of the
topmost mark (but not pop it) and call the C function pointed to by that
...
this document. Finally, the C<ENTERSUB> OP will do any post-processing of
...
that has an initial C<SvREFCNT()> of one. This is actually one too high:
the expectation is that the SV will shortly be embedded somewhere, such as
stored in an array, which will take "ownership" of that one count. If the
program calls C<croak()> or similar before the SV has been embedded, then
it will leak. Note that C<croak()> can be trapped by C<eval()>, so it's
possible that C<croak()> could be called many times, leaking each time.
Note also that many things may indirectly trigger a C<croak()>. For
...
and pushes it onto the stack. The C<ENTERSUB> OP which typically invokes
...
and body fields over the years. Always use the macros. For example,
C<SvIVX(sv)> directly accesses the IV field of the SV (which may be in the
head or body depending on the SV's type). If the SV has a valid integer
value, then the C<SVf_IOK> flag will be set, which can be tested with the
macro C<SvIOK(sv)>.
...
In total, the C<SvIV(sv)> macro does roughly the equivalent of:
    ...
    if (SvGMAGICAL(sv))
        mg_get(sv);   // call FETCH etc; update the SV's value and flags
    if (!SvIOK(sv))
...
C<SvIVX()> etc without testing for C<SvIOK()> first may access a field in
a body which doesn't exist and thus trigger a SEGV.
...
skip a second magic call with variants like C<SvIV_nomg()>.
...
The SV structure can also be used to store things which aren't scalar
values: in particular, arrays, hashes and code values. There are typedefs
for AV, HV and CV structures (plus a few others). These structures are
identical to SVs and can generally be used interchangeably with suitable
casting, e.g. C<SV *ret = (SV*)av>. The main feature of these non-scalar
SVs is that the value of the type field in these cases, C<SVt_PVAV>,
C<SVt_PVHV>, C<SVt_PVCV> etc, actually I<do> indicate the Perl type,
rather than just indicating what sort of body they have.

An important thing to note is that such non-scalar SVs are never directly
pushed onto the stack when calling and returning from subroutines and
XSUBs. Instead, where necessary, references to them are pushed.
...
characters each using two bytes. Once the third character is removed, the
string stays in UTF-8 encoding, with C<SvCUR(sv) == 3, sv_len_utf8(sv) == 2>
and the second character using two bytes. So a string SV when passed to an
XSUB has two possible representations; and which will be used is somewhat
unpredictable.
...
a single byte, then the call croaks with a C<Wide character> error.
Conversely, C<SvPVutf8()> and variants force the string to UTF-8.
...
This XSUB definition translates into a C function whose start may look
something like this (the exact details may vary across releases):

    void XS_Foo__Bar_baz(pTHX_ CV* cv)
    {
        dVAR; dXSARGS;
        if (items < 1 || items > 2)
...
The C function's name is based on the XSUB's name plus the current package
(with C<s/:/_/g>). Apart from debugging, you don't generally need to know
this name.
...
For C<CODE> and autocall, unless the return type is void, the parser can
generate code to return the value of C<RETVAL>. This is automatic in the
case of autocall, but for C<CODE> you have to tell the parser to do so with
C<OUTPUT: RETVAL>. The code generated in either case may look something
like
...
attached to the calling C<ENTERSUB> OP may be used instead of allocating
...
Typically an XSUB's declared return type is a C type such as C<int> or
...
C<PL_sv_undef> (which is what the Perl function C<undef> returns) to be
stored at C<ST(0)>, and then to immediately return.
...
require typmap conversions (e.g. it always returns via C<XSRETURN_YES> or
...
Here is where some special processing comes in. The XS compiler, in the
...
the XSUB (usually the C<ENTERSUB> OP) notes the position of the base of
the argument stack frame before calling the XSUB, and the position of the
...
typically created automatically with C<h2xs> when you first create the
...
C<ExtUtils::ParseXS> XS compiler (and its B<xsubpp> wrapper) needed to
...
C<Makefile> as C<XS_VERSION>, and from there, is baked into the boot XSUB.
When the module is loaded and the boot code called, the versions are
compared, and it croaks if there's a mismatch. This usually indicates that
the C<.so> and C<.pm> files are from different installs: for example
someone copied over a more recent version of the C<.pm> file but forgot to
copy or rebuild the C<.so>.

If the version of the PM module is a floating point number, it will be
stringified with a possible loss of precision (currently chopping to nine
decimal places), so  it may not match the version of the XS module any
more. Quoting the C<$VERSION> declaration to make it a string is
...
In general, XSUB prototypes, like perl sub prototypes, are of very limited
use and are typically only used to mimic the behaviour of Perl builtins.
For example there is no way to implement a C<push @a, ...;> style function
without a way of telling the Perl interpreter not to flatten C<@a>.
Outside of these  narrow uses, it is generally a mistake to use
prototypes.
...
then the compiler will interpret the parameters as a command. This feature
is mildly deprecated in favour of the C<INCLUDE_COMMAND:> directive, as
documented below. The latter can be used to ensure that the perl (if any)
used in the command is the same as the one running the XS parser.
...
Since 5.8.1. It defaults to C<UNDEF> for each package. It sets the default
fallback handling behaviour for overloaded methods in the current package
(i.e. C<Foo:Bar> in the example above). It is analogous to the Perl-level:
...
something starting on column one which isn't otherwise recognised
as an XSUB keyword or file-scoped directive.
...
keywords, which specify the main C code body of the XSUB, and how its
parameters and any any return value should be processed. Other keywords
can change the behaviour of the XSUB, or affect how it is registered with
Perl, e.g. with extra named aliases. In the absence of an explicit main C
code body specified by the C<CODE> or C<PPCODE> keywords, the parser will
generate a body automatically; this is referred to as
L<autocall|/"Auto-calling a C function"> in this document.
...
An XSUB's body can be thought of as having up to five parts, depending on
which keywords are permissible at that part of the XSUB. These are the
L<Input|/"The XSUB Input Part">, L<Init|/"The XSUB Init Part">,
L<Code|/"The XSUB Code Part">, L<Output|/"The XSUB Output Part"> and
L<Cleanup|/"The XSUB Cleanup Part"> parts. There is no formal syntax to
define this structure; it's just an understanding that certain keywords
may only appear after certain other keywords etc.
...
=head2 An XSUB Declaration
    ...
    # All on one line;  param default value

    int foo2(int a, char *s = "")

    # Complex parameters, with variable argument length

    int
    foo3(OUT int a, IN_OUTLIST char *s, STRLEN length(s), ...)
    ...
    # Various return type qualifiers; C++ method

    NO_OUTPUT extern "C" static int
    X::Y::foo5(int a, char *s) const
...
error condition indicator. For example,
    ...
    delete_file(char *name)
      # implict autocall code here: RETVAL = delete_file(name);
      POSTCALL:
        if (RETVAL != 0)
...
Some examples of valid XSUB parameter declarations:
    ...
    OUT char *foo         # arg gets updated
    IN_OUTLIST char *foo  # parameter gets returned
...
The most straightforward type of declaration an XSUB's parameter list
...
that name to be declared; and second, that variable is initialised to the
...
Apart from the variable declaration and initialisation, the name of the
parameter will usually be used in the usage message and in any autocall,
as shown above. These variables are accessible for any user code in a
C<CODE> block or similar. Their values aren't normally returned.
...
parameter declaration which wasn't parseable. This inadvertently made many
...
An input part of C<IN> causes that variable to be initialised to the
value of the corresponding passed argument (the default). Otherwise the
initialisation is skipped. In particular, this means that an autocall
...
an initialiser expression, to the C<INPUT> line.
    ...
    foo(a,b,c,d,e,f,g)
        ...
        # Add deferred initalisation code
...
The SCOPE keyword may be either XSUB-scoped for file-scoped (this refers
to the scope of the keyword within the XS file, not to the scope generated
by the keyword). For the first, it may appear anywhere in the input part
or the XSUB. For the latter, it may appear anywhere in file scope, but due
to a long-standing parser bug, the keyword's state is reset at the start
of each XSUB, so it will only have any effect if appears just before a
...
by perl-5.12.0 (xsubpp v2.21) and fixed in perl-5.44.0 (xsubpp v3.58). The
file-scoped form has been available since perl-5.12.0 .
...
old-style C<INPUT> lines rather than an ANSI-style declaration, i.e.
C<foo(int i)>. In fact, the XS parser, when looking for a SCOPE comment in
a typemap, is currently very lax: it's actually a case-insensitive match
of any code comment which contains the text "scope" plus anything else.
But you shouldn't rely on this; always use the form shown here.
...
the parentheses in the autocall. This is useful when the ordering etc of
parameters vary between Perl and C, without a need to write a C<CODE> or
C<PPCODE> section.
...
XSUB, or since 5.40.0, only into any declarations within the same case
C<CASE> branch.
...
text is I<not> variable expanded. It is thus very hard to ensure that the
...
swap can in fact be undef in addition to false, to indicate an assign
overload such as C<+=>.
...
to add overloading:
    ...
    mynum_add(My::Num x, My::Num y, bool swap)
     ...
     # and three similar XSUBs for mynum_subtract, mynum_multiply,
     # mynum_divide
...
Sometimes you end up writing several XSUBs which are very similar: they
...
Note that this keyword is deprecated as it assumes a particular
...
the typemap file included with the distribution adding to (and overriding)
...
extracted (perhaps converting a string value like C<"123"> to an IV), then
that value will be cast to the final C type, with the usual C rules for
casting between integer types. Conversely when returning a value, the C
value is first cast to C<IV>, then the SV is set to that value.
...
string value (or can be converted to a string value), or may be a
...
C<"ARRAY(0x12345678)">. When returning the value of a C<T_PV> type, the C
string is copied to the temporary SV which is to be returned, with its
length being determined by C<strlen()> or its equivalent. See L</Unicode
and UTF-8> for the difficulties associated with handling UTF-8 strings.
...
using aliasing:
    ...
    My::Num
    arthmetic_interface(My::Num x, My::Num y)
        INTERFACE:
            mynum_add
            mynum_subtract
            mynum_multiply
